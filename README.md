...
| `polkit-gnome` | Session | GUI authentication prompts; started via `exec-once` in hyprland.conf |
...
| `foot` | Terminal | `$terminal` in hyprland.conf, bound to SUPER+Return |
...
| `stow` | CLI | Symlinks `~/dotfiles_hyprland/*` into `~/.config` — every config is a stow symlink |
